...
def fetch_logs(w3, start_block, event, handle_logs, block_range=17280):
    ...
    while start_block < latest.number:
        try:
            ...
            if len(logs) > 0:
                ...
                print(
                    f"got {len(logs)} logs in ({round(response_time)}s) (up to block: {end_block} {delta.days} days ago)"
                )
                handle_logs(logs)
            ...
            if response_time > 5:
                ...
                print(
                    f"long reponse ({round(response_time)}s), reducing block_range to: {block_range}"
                )
            elif response_time < 1:
                block_range += round(block_range / 10)
                print(
                    f"short reponse ({response_time}s), increasing block_range to: {block_range}"
                )
    ...
    while True:
        ...
        if latest.number >= start_block:
            ...
            if len(logs) > 0:
                print(
                    f"got {len(logs)} log(s) in the last {latest.number - start_block} block(s)"
                )
                handle_logs(logs)
            start_block = latest.number + 1
        time.sleep(5)
